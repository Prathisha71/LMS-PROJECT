...
export const AssignmentPage: React.FC = () => {
  const { profile, boards, assignments, submitAssignment, setView } = useLmsStore();
  ...
  const [selectedAssignId, setSelectedAssignId] = useState(classAssignments[0]?.id || '');
        ...
        {activeAssign ? (
          <div className="glass-card p-6 border-slate-200 dark:border-white/5 text-left space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-white/5 pb-5">
              <div>
                <span className={`text-[10px] font-bold uppercase tracking-wider ${subjectTagColor}`}>
                  {activeAssign.subjectTitle} Homework Sheet
                </span>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mt-1">{activeAssign.title}</h3>
              </div>
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-white/5">
              {activeAssign.status === 'Pending' ? (
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  <h4 className="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wider">File Submission</h4>
      ...
      <div className="space-y-6">
        ...
        <div className="glass-card p-5 border-slate-200 dark:border-white/5 space-y-4">
          <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-3">
            ...
            <h3 className="text-xs font-bold text-slate-700 dark:text-slate-400 uppercase tracking-widest text-left">Homework Log</h3>
          </div>

          <div className="space-y-2.5">
            {classAssignments.length === 0 ? (
              <div className="text-center py-8 text-xs text-slate-500 dark:text-slate-500 font-semibold">
                No homework logs available for Class {profile.selectedClassId.replace('class-', '')}
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center w-full gap-2">
                      <span className={`text-xs font-bold truncate max-w-[150px] ${isSelected ? 'text-brand-royal dark:text-white' : 'text-slate-800 dark:text-slate-300'}`}>{a.title}</span>
